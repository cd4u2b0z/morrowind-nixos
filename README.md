...
| **Stylix** | System-wide declarative theming |

### Theme (Managed by Stylix)
- **Color Scheme**: Nord (base16) - easily changeable!
- **GTK Theme**: Auto-generated by Stylix
- **Icons**: Papirus-Dark
- **Cursor**: Nordzy-cursors-white
- **Font**: Inter, JetBrainsMono Nerd Font

> 💡 **Theming**: Unlike Arch with wallust (runtime theming), NixOS uses **Stylix** for declarative theming. 
> Change your entire system theme by editing ONE line in `modules/stylix.nix` and rebuilding.
> See [Changing Themes](#-changing-themes-stylix) for details.
...
│   └── stylix.nix                     # 🎨 THEMING - Edit this to change colors!
├── home/
...
### 🎨 Changing Themes (Stylix)

Unlike Arch Linux with wallust (live theme switching), NixOS uses **Stylix** for declarative theming.
This means: **edit one file, rebuild, entire system matches**.

**To change your theme:**

1. Edit `~/nixos-config/modules/stylix.nix`:

```nix
# Change this line (around line 24):
base16Scheme = "${pkgs.base16-schemes}/share/themes/nord.yaml";

# To any of these (or many more!):
...
**That's it!** Kitty, Waybar, Mako, Fuzzel, Hyprland borders, GTK apps, Neovim - ALL themed.

**Available themes:** Run `ls $(nix-build '<nixpkgs>' -A base16-schemes)/share/themes/` to see all ~200 options.

### Change Wallpaper

Wallpaper is handled by swww (still works for animated transitions):

```bash
# Set wallpaper with animation
swww img ~/Pictures/Wallpapers/your-image.jpg --transition-type grow

# Use wallpaper picker
~/.config/hypr/scripts/quick-wallpaper.sh select

# Set random wallpaper
~/.config/hypr/scripts/quick-wallpaper.sh random
```

> **Note:** Stylix sets the initial wallpaper from `modules/stylix.nix`. 
> You can change it at runtime with swww, but it will reset on reboot.
> To change permanently, edit the `image = ` line in `modules/stylix.nix`.

### Add New Packages

Edit `~/nixos-config/modules/packages.nix`:
...
Edit `~/nixos-config/home/dotfiles/hypr/hyprland.conf`:

```bash
# Find the keybinds section and add:
